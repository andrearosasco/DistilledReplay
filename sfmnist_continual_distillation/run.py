...
def run(config):
    ...
    if log_config['wandb']:
        wandb.init(project="sfmnist-cvpr", name=log_config['wandb_name'])
    ...
    for task_id, task in enumerate(run_config['tasks'], 0):
        ...
        validset = Dataset(dset='test', valid=data_config['valid'], transform=data_config['test_transform'],
                           classes=task)
        validloaders.append(DataLoader(validset, batch_size=param_config['batch_size'], shuffle=False, pin_memory=True,
                                       num_workers=data_config['num_workers']))
        trainset = Dataset(dset='train', valid=data_config['valid'], transform=data_config['train_transform'],
                           classes=task)
        trainloader = DataLoader(trainset, batch_size=param_config['batch_size'], shuffle=True, pin_memory=True,
                                 num_workers=data_config['num_workers'])
        ...
        d_trainloader = DataLoader(trainset, batch_size=param_config['distill_batch_size'], shuffle=True,
                                   pin_memory=True, num_workers=data_config['num_workers'])
        d_validloader = DataLoader(validset, batch_size=param_config['distill_batch_size'], shuffle=False,
                                   pin_memory=True, num_workers=data_config['num_workers'])
        # DEBUG
        ...
        # split_sets = torch.utils.data.random_split(trainset, [int(len(trainset) / param_config['buffer_size']) for _ in
        # range(param_config['buffer_size'])])

        # for s_set in split_sets:
        #     b = None
        ...
        if param_config['buffer_size'] != 0:
            ...
            if log_config['wandb']:
                mean, std = data_config['test_transform'].transforms[-1].mean, data_config['test_transform'].transforms[
                    -1].std
                x, y = next(iter(DataLoader(buffer, batch_size=len(buffer), shuffle=False)))
                print_images(x[:1], y[:-1], mean, std)
                print_images(x[1:], y[-1:], mean, std)

            # s_loader = DataLoader(s_set, batch_size=param_config['distill_batch_size'], shuffle=True,
            #                        pin_memory=True, num_workers=data_config['num_workers'])

            buffer, _ = distill(d_net, buffer, memories, config, criterion, d_trainloader, d_validloader, task_id)
...
def distill(model, buffer, memories, config, criterion, train_loader, valid_loader, id):
    ...
    # all_imgs, all_trgs = None, None
    # for m in memories:
    #
    #     mem_imgs, mem_trgs = next(iter(DataLoader(m, batch_size=len(m), shuffle=False, pin_memory=True)))
    #     mem_imgs, mem_trgs = mem_imgs.to(run_config['device']), mem_trgs.to(run_config['device'])
    #
    #     all_imgs = mem_imgs if all_imgs is None else torch.cat((all_imgs, mem_imgs))
    #     all_trgs = mem_trgs if all_trgs is None else torch.cat((all_trgs, mem_trgs))
            ...
            for r, sigma in enumerate(init_batch):
                ...
                with higher.innerloop_ctx(model, model_opt) as (fmodel, diffopt):
                    for j in range(param_config['inner_steps']):
                        ...
                        #     buff_out = fmodel(all_imgs)
                        #     buff_loss = criterion(buff_out, all_trgs)
